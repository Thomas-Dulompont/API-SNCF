...
geo = load_geo()
objects_par_regions = load_objects_par_regions()
#########################################
#########################################
...
@st.cache(allow_output_mutation=True)
def load_df_weather_by_choice():
    df_weather_by_choice = pd.read_csv(f"CSV/weather_by_dep_by_{choice}.csv")
    return df_weather_by_choice
df_weather_by_choice = load_df_weather_by_choice()




@st.cache(allow_output_mutation=True)
def load_df_choice_total():
    df_choice_total = pd.read_csv(f'CSV/df_{choice}_total.csv')
    return df_choice_total
df_choice_total = load_df_choice_total()


@st.cache(allow_output_mutation=True)
def load_df_choice_by_type():
    df_choice_by_type = pd.read_csv(f'CSV/df_{choice}_by_type.csv')
    return df_choice_by_type
df_choice_by_type = load_df_choice_by_type()

if choice in ['day', 'week']:
    @st.cache(allow_output_mutation=True)
    def load_objet_region_by_choice():
        objet_region_by_choice = pd.read_csv(f'CSV/objects_by_region_by_{choice}.csv')
        return objet_region_by_choice
    objet_region_by_choice = load_objet_region_by_choice()
...
with mygrid2[0][2]:
    st.metric(f"Max items lost in :", df_choice_total.sort_values('nb_objets', ascending=False).iloc[0,0])
...
with col2:
    ...
    if choice in ['day', 'week']:
        st.warning('Attention : Brut values presented in this map', icon="⚠️")
